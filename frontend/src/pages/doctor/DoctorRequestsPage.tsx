                ...
                {requests.map((req) => (
                  <tr key={req.id} className="transition-colors hover:bg-gray-50">
                    <td className="px-6 py-4">
                      {req.patient ? (
                        <div>
                          <div className="font-extrabold text-gray-900">{req.patient.fullName}</div>
                    ...
                    <td className="px-6 py-4 font-bold text-slate-600">{req.patient?.diabetesType || '—'}</td>
                    <td className="px-6 py-4 font-bold capitalize text-slate-600">{req.patient?.managementType || '—'}</td>
                    <td className="max-w-xs truncate px-6 py-4 text-slate-600">{req.message || '—'}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-2 text-xs font-extrabold text-emerald-700 transition-colors hover:bg-emerald-100 disabled:opacity-50"
                        >
                          Accept
                        </button>
                        <button
                          ...
                          className="rounded-lg border border-red-200 bg-red-50 px-4 py-2 text-xs font-extrabold text-red-700 transition-colors hover:bg-red-100 disabled:opacity-50"
                        >
                          Reject
                        </button>
                      </div>
