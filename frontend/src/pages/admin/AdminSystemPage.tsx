    ...
    <AdminLayout>
      <header className="relative overflow-hidden rounded-3xl border border-gray-200 bg-gray-50/40 px-5 py-5 shadow-[0_18px_48px_rgba(0,0,0,0.4)] backdrop-blur-3xl sm:px-7 xl:px-8 xl:py-7">
        ...
        <div className="relative">
          ...
          <p className="mt-4 text-[clamp(1rem,1.25vw,1.25rem)] leading-7 font-bold text-slate-600">Monitor server performance and system configuration.</p>
        </div>
      </header>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:gap-6">
        {cards.map((c) => (
          <div key={c.label} className="group relative overflow-hidden rounded-2xl border border-gray-200 bg-gray-50/40 p-5 shadow-[0_14px_34px_rgba(0,0,0,0.3)] backdrop-blur-2xl transition-all duration-300 hover:-translate-y-1 hover:border-violet-500/40 xl:p-6">
            <div className="relative">
              <div className="flex items-center gap-2">
                <p className="text-sm font-extrabold text-slate-500">{c.label}</p>
      ...
      <section className="relative overflow-hidden rounded-2xl border border-gray-200 bg-gray-50/40 p-5 shadow-[0_18px_46px_rgba(0,0,0,0.3)] backdrop-blur-2xl xl:p-7">
        <h2 className="mb-5 text-lg font-extrabold text-gray-900">Memory Usage</h2>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {[
            ...
            <div key={m.l} className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
              <div className="mb-2 flex justify-between"><span className="text-sm font-bold text-slate-500">{m.l}</span><span className="text-sm font-extrabold text-gray-900">{m.v} MB</span></div>
              <div className="h-2.5 overflow-hidden rounded-full bg-gray-50"><div className="h-full rounded-full bg-gradient-to-r from-violet-500 to-fuchsia-500" style={{ width: `${Math.min((m.v / m.mx) * 100, 100)}%` }} /></div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="relative overflow-hidden rounded-2xl border border-gray-200 bg-gray-50/40 p-5 shadow-[0_18px_46px_rgba(0,0,0,0.3)] backdrop-blur-2xl xl:p-7">
        <h2 className="mb-5 text-lg font-extrabold text-gray-900">Details</h2>
        <div className="divide-y divide-white/5">
          {[['Node.js', info.nodeVersion], ['Platform', info.platform], ['Database', info.databaseMode], ['Env', info.environment], ['Uptime', fmtUp(info.uptime)]].map(([l, v]) => (
            <div key={l} className="flex justify-between py-3"><span className="text-sm font-bold text-slate-500">{l}</span><span className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-1 text-sm font-extrabold text-gray-900">{v}</span></div>
          ))}
        </div>
      </section>
    </AdminLayout>
