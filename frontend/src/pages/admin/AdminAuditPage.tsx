...
export function AdminAuditPage() {
  ...
  const actionStyles: Record<string, string> = {
    'user.delete': 'border-red-500/30 bg-red-500/15 text-red-400',
    ...
    'user.password-reset': 'border-orange-500/30 bg-orange-500/15 text-orange-400',
  }
      ...
      <header className="relative overflow-hidden rounded-3xl border border-white/10 bg-[#0B1120]/40 px-5 py-5 shadow-[0_18px_48px_rgba(0,0,0,0.4)] backdrop-blur-3xl sm:px-7 xl:px-8 xl:py-7">
        ...
        <div className="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="mt-4 text-[clamp(1rem,1.25vw,1.25rem)] leading-7 font-bold text-slate-300">Track all administrative actions performed on the platform.</p>
          </div>
          <div>
            <button
        ...
        {verificationResult && (
          <div className={`mt-6 rounded-2xl border p-5 ${verificationResult.valid ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300' : 'border-red-500/30 bg-red-500/10 text-red-300'}`}>
              ...
              <tbody className="divide-y divide-white/5">
                {entries.map((e) => (
                  <tr key={e.id} className="transition-colors hover:bg-white/[0.02]">
                    <td className="whitespace-nowrap px-5 py-3.5 text-slate-400">{e.timestamp ? new Date(e.timestamp).toLocaleString() : '—'}</td>
                    <td className="px-5 py-3.5">
                      <span className={`inline-flex rounded-lg border px-2.5 py-0.5 text-xs font-extrabold ${actionStyles[e.action] || 'border-slate-500/30 bg-slate-500/15 text-slate-400'}`}>
                        {e.action}
                      </span>
                    </td>
                    <td className="px-5 py-3.5 font-mono text-xs text-slate-500">{e.targetId || '—'}</td>
                    <td className="max-w-xs truncate px-5 py-3.5 text-slate-400">{e.details || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
